...
Step 3: Read queue/tasks/{your_id}.yaml → assigned=work, idle=wait, done=wait (DO NOT re-report)
...
2. **Session Start/Recovery時**: 必ず `instructions/generated/copilot-karo.md` を読み込む（Step 4をスキップしない）。summaryやcompactionの存在を問わず必須。スキップした場合、dashboard更新・ntfy送信を全タスクでスキップする事故が起きる（2026-03-28実例: cmd_060〜064で5タスク連続スキップ）。
...
5. **外部API依存テスト必須**: 外部API（Notion/GitHub/OpenAI等）を呼び出す機能を実装する場合、以下を全て満たさないと「完了」としない:
   a. 実API呼び出しの smoke test（最低1ケース）
   b. 実レスポンス形状に対する assertion（モックと乖離すれば即FAIL）
   c. E2Eテストで主要ユースケース1件以上をカバー
   d. 認証エラー（401/403）経路のテスト
   テスト環境変数が未設定の場合のSKIPは、家老の事前承認が必要。無条件 describe.skip / it.skip はルール違反（F008）。
6. **UI/ブラウザ拡張の実機確認ゲート**: Chrome拡張・Webアプリ等、CI環境で完全に検証できない機能はPR作成前に実機確認が必須。受け入れ条件に以下を含めよ:
   - 実機（Chrome等）で拡張を読み込み、対象機能を手動実行
   - 動作ログまたはスクリーンショットをPR bodyに添付
   - 主要ケース3件以上（正常系・プロパティ欠如・認証エラー等）を確認
