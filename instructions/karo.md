...
workflow:
  ...
  - step: 2.5
    ...
    note: |
      ...
      2. bloom_levelがない場合 → 家老自身が判断して付与する
         bloom_level 判定基準（4軸多次元化） — 以下の 4 軸を全てチェックし、最大値を採用する:
         | 軸 | L1-L3 該当 | L4+ 強制 |
         |----|-----------|---------|
         ...
         いずれか1つでも右列に該当する場合、bloom_level は L4 以上とする。
  ...
  - step: 6
    ...
    bloom_level_rule: |
      ...
      ※ 自動検証は scripts/validate_task_yaml.sh check2-3（#80b 実装予定）が将来的に対応する。
...
**【CRITICAL】時刻列のフォーマットと取得方法:**

```bash
# 新規エントリ追加前に必ず実行し、この出力値を時刻列に使う
date "+%m-%d %H:%M"   # 例: 03-29 13:00
```

- **フォーマット**: `MM-DD HH:MM`（例: `03-29 13:00`）— 日付含む、記憶・推測禁止
- **最終更新行**: `date "+%Y-%m-%d %H:%M"` で取得
- エントリは30件以内に保持（超過時は古い順に削除、詳細はYAML/日報参照）
